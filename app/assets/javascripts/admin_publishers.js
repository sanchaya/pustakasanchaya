// Admin Metadata - Publishers
document.addEventListener('DOMContentLoaded', initPublishersMetadata);
document.addEventListener('turbolinks:load', initPublishersMetadata);

function initPublishersMetadata() {
...
function submitRename() {
  ...
  fetch(adminPublishersPaths.rename, {
...
function findSimilarPublishers() {
  ...
  fetch(adminPublishersPaths.findSimilar + '?name='+encodeURIComponent(name), {
...
function submitMerge() {
  ...
  fetch(adminPublishersPaths.merge, {
    method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-Token':document.querySelector('meta[name="csrf-token"]').content},
    body: JSON.stringify({ old_name: oldName, new_name: newName })
  }).then(function(r){return r.json()}).then(function(data){
    if (data.success) {
...
function submitMergeMultiple() {
  ...
  fetch(adminPublishersPaths.mergeMultiple, {
    method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-Token':document.querySelector('meta[name="csrf-token"]').content},
