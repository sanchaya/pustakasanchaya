// Admin Metadata - Libraries
(function() {
  document.addEventListener('DOMContentLoaded', initLibrariesMetadata);
  document.addEventListener('turbolinks:load', initLibrariesMetadata);

  function initLibrariesMetadata() {
    if (!document.getElementById('renameModal')) return;
  }

  let renameModal, mergeModal, librariesMergeMultipleModal;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    librariesMergeMultipleModal = new bootstrap.Modal(document.getElementById('librariesMergeMultipleModal'), { keyboard: false });

    document.addEventListener('click', function(e) {
      if (e.target.closest('.rename-btn')) {
        e.preventDefault();
        var btn = e.target.closest('.rename-btn');
        var libraryName = btn.getAttribute('data-category-name');
        openRenameModal(libraryName);
      }
  ...
  function submitRename() {
    ...
    fetch(adminLibrariesPaths.rename, {
  ...
  function findSimilarLibraries() {
    ...
    fetch(adminLibrariesPaths.findSimilar + '?name='+encodeURIComponent(name), {
  ...
  function submitMerge() {
    ...
    fetch(adminLibrariesPaths.merge, {
      method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-Token':document.querySelector('meta[name="csrf-token"]').content},
      body: JSON.stringify({ old_name: oldName, new_name: newName })
    }).then(function(r){return r.json()}).then(function(data){
      if (data.success) {
  ...
  function openMergeMultipleModal() {
    ...
    librariesMergeMultipleModal.show();
  }
  ...
  function submitMergeMultiple() {
    ...
    fetch(adminLibrariesPaths.mergeMultiple, {
      method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-Token':document.querySelector('meta[name="csrf-token"]').content},
