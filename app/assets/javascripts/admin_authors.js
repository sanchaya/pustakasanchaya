// Admin Metadata - Authors
(function() {
  document.addEventListener('DOMContentLoaded', initAuthorsMetadata);
  document.addEventListener('turbolinks:load', initAuthorsMetadata);

  function initAuthorsMetadata() {
    if (!document.getElementById('renameModal')) return;
  }

  let renameModal, mergeModal, authorsMergeMultipleModal;
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    authorsMergeMultipleModal = new bootstrap.Modal(document.getElementById('authorsMergeMultipleModal'), { keyboard: false });

    document.addEventListener('click', function(e) {
      if (e.target.closest('.rename-btn')) {
        e.preventDefault();
        var btn = e.target.closest('.rename-btn');
        var authorName = btn.getAttribute('data-category-name');
        openRenameModal(authorName);
      }
  ...
  function submitRename() {
    ...
    fetch(adminAuthorsPaths.rename, {
  ...
  function findSimilarAuthors() {
    ...
    fetch(adminAuthorsPaths.findSimilar + '?name='+encodeURIComponent(name), {
  ...
  function openMergeMultipleModal() {
    ...
    authorsMergeMultipleModal.show();
  }
  ...
  function submitMergeMultiple() {
    ...
    fetch(adminAuthorsPaths.mergeMultiple, {
      method: 'POST', headers: {'Content-Type':'application/json','X-CSRF-Token':document.querySelector('meta[name="csrf-token"]').content},
